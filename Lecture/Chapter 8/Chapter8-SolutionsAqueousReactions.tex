...
\begin{frame}{Solution Terminology}
	\begin{columns}
		...
		\column{0.4\textwidth}
		\begin{center}
			\includegraphics[scale=0.45]{Borland-Solute-Solvent.jpg}
		\end{center}
	\end{columns}
...
\begin{frame}{Some Notes on Solutes}
	\begin{columns}
		...
		\column{0.5\textwidth}
		\begin{center}
			\includegraphics[scale=0.4]{Borland-Solutes.jpg}
		\end{center}
	\end{columns}
\end{frame}
...
\begin{frame}{Diluting Solutions}
	...
	\only<2->{%
		...
		\begin{align*}
			...
			\uncover<3>{\intertext{But this actually applies to \alert{any} units of
			concentration or volume, so}
			C_1V_1 &= C_2V_2}
		\end{align*}
	}
\end{frame}
...
\frame{\section{Predicting Reaction Products}
	\begin{learningobjectives}
	...
	\item Identify when a product will be in a different state (gas).
	\item Follow the transfer of electrons in oxidation-reduction reactions.
	\end{learningobjectives}
}
...
\begin{frame}[t]{Solution Stoichiometry}
	...
	\begin{overprint}
		\onslide<1-3>
		Did \alert{all} of the \ch{Ba(NO3)2} and \ch{Na3PO4} react?
		\begin{itemize}[<+(1)->]
			\item We would need \alert{exactly} the correct amount of
				\ch{Ba(NO3)2} and \ch{Na3PO4}.
			\item What is the correct amount?
		\end{itemize}

		\onslide<4->
	
		What volume (in \si{\milli\liter}) of a \SI{0.200}{\Molar} \ch{Na3PO4} solution
		is required to completely react with \SI{10.00}{\milli\liter} of a
		\SI{0.025}{\Molar} solution of \ch{Ba(NO3)2}?
...
\begin{frame}[t]{Writing Electrolyte Dissociation Reactions}
	...
	\begin{enumerate}
		...
		\item \ch{HF\aq{} ->[ H2O ]}
			...
			F^{-}\aq{}}}
			\vspace{2em}
		\item \ch{C2H5OH\aq{} ->[ H2O ]}
			\note[item]{\ch{C2H5OH\aq{} -/>[ H2O ]}}
			\vspace{2em}
	\end{enumerate}
\end{frame}
...
\begin{frame}{Some notes on acids\ldots}
	\begin{itemize}[<+->]
		\item \ch{H+} normally associates with water to form the
			\alert{hydronium ion}.
			\begin{reaction*}
...
\begin{frame}{Naming Acids}
	\begin{itemize}
		\item \alert{Binary acids} have just an \ch{H} and a nonmetal.
			\begin{itemize}
			...
			\pause
		\item \alert{Oxyacids} contain \ch{H} and a polyatomic anion
...
\begin{frame}{Guide to Balancing Acid-Base Reactions}
	...
	\begin{description}[<+(1)->]
		...
		\item[Step 2] Balance \Hyd\ and \ch{H^{+}}.
...
\begin{frame}{Practical Lab Skills: Titrations}
	...
	\begin{center}
		\only<1>{\includegraphics[scale=0.35,trim={0 0 0 30pt},clip]{08_18_Figure.jpg}}
		\mode<article>{\clearpage}
		\only<2>{\includegraphics[scale=0.4]{08_19_Figure.jpg}}
	\end{center}
	\end{overlayarea}
\end{frame}
