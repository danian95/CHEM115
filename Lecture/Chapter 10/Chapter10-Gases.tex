...
\begin{frame}{Gas Pressure}
	...
	\begin{block}{Pressure}
		The force acting on a specific area.
		\begin{equation*}
			\text{pressure ($P$)} =
...
\only<2|article:0>{%
	\begin{center}
		\includegraphics[scale=0.4]{10_01_Table.jpg}
	\end{center}
}
\end{frame}
...
\begin{frame}{Methods of Measuring Pressure}
	\only<1>{%
	\begin{columns}
		\column{0.5\textwidth}
...
\begin{frame}[t]{Boyle's Law}
	%\begin{overlayarea}{\linewidth}{25em}
		\begin{itemize}[<+->]
...
\begin{frame}{Boyle's Law -- A Diver's View}
	...
	\only<2|article:0>{%
	\begin{center}
		\includegraphics[scale=0.5]{10_12_Figure.jpg}
	\end{center}
}
\end{frame}
...
\begin{frame}[t]{Gas Law Trends}
	Using gas laws, complete the following statements with \alert{increases}
	or \alert{decreases}.
