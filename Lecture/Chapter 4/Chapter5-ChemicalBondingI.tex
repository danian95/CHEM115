...
\begin{frame}{Beyond the Lewis Model}
	\begin{itemize}[<+->]
		...
		\item Both \ch{H2} and \ch{HF} \emph{share} the same number of
			electrons:
...
\begin{frame}{Bond Polarity}
	\begin{itemize}[<+->]
		\item Many more electrons are surrounding F in HF -- an
			...
			\begin{center}
				\includegraphics[scale=0.2]{05_02_Figure.jpg}
			\end{center}
...
\begin{frame}{The Dipole Moment}
	A dipole moment represents both \emph{magnitude} and \emph{direction}.
	\begin{itemize}
		\item The \emph{magnitude} can be calculated as
			\begin{equation*}
			...
			\begin{description}
				...
				\item[$q$] is the \emph{magnitude} of equal (but
					opposite) charges, and
				\item[$r$] is the distance over which the
					charges are separated.
			\end{description}
		\item The \emph{direction} is always \emph{towards} the more
			negative charge.
	\end{itemize}
	...
	\begin{tikzpicture}[remember picture, overlay]
		...
		at ($(pic cs:charge) + (1,-0.5)$) {In our case, $q$ is the charge
...
\begin{frame}{Percent Ionic Character}
	...
	covalent bonds, we can estimate the \emph{percent ionic character}:
...
\begin{frame}{Incomplete Octets}
	\begin{itemize}[<+->]
		...
		\item For a compound such as \ch{BF3}, we \emph{could} create a
			double bond to form a complete octet around boron.

			\begin{center}
				\includegraphics[scale=0.1]{05_Pg218_UnFigure_2.jpg}
			\end{center}
		\item This would create a positive formal charge on \ch{F},
			which is \emph{very unfavorable}.

			\begin{center}
				\includegraphics[scale=0.1]{05_Pg218_UnFigure_3.jpg}
			\end{center}
	\end{itemize}
...
\begin{frame}[allowframebreaks]{Six Electron Groups}
	...
	\begin{itemize}
		...
		\item \emph{All} bond angles are \SI{90}{\degree}
	\end{itemize}
...
\begin{frame}{Shapes of Larger Molecules}
	...
	\begin{columns}
		...
		\column{0.45\linewidth}
		\centering
		\chemfig{\lewis{5:,N}~[:45]C-[:45]Si(-[:45]\lewis{1:3:7:,F})(-[:135]\lewis{1:3:5:,F})(<[:260]\lewis{0:4:6:,F})(>:[:80]\lewis{0:2:4:,F})-[:315]C
	(-[:225]H)(-[:315]H)-[:45]C(=[:90]\lewis{1:3:,O})-[:315]\lewis{5:7:,O}-[:45]H}
	\end{columns}
\end{frame}
...
\begin{frame}{Polar Molecules}
	...
	\begin{center}
		\chemfig{H-Cl}
		\hspace{5em}
		\chemfig{H>:[:20]\lewis{2:,N}(<[:240]H)-[:330]H}
	\end{center}
\end{frame}
	...
	\begin{itemize}
		...
		\item \emph{May} have a symmetrical arrangement of polar bonds.
			...
			\begin{center}
				...
				\hspace{5em}
				\chemfig{\lewis{2:4:6:,Cl}>:[:20]C(<[:240]\lewis{3:5:7:,Cl})(-[:90]\lewis{0:2:4:,Cl})-[:330]\lewis{0:2:6:,Cl}}
			\end{center}
	\end{itemize}
\end{frame}
...
\begin{tikzpicture}[node distance = 8em, auto]
	% Place nodes
	\node [block] (molecule) {Molecule};
	\node [block, below of=molecule] (centrallonepairs) {Only 1 lone pair on central atom?};
	\node [block, left of=centrallonepairs, yshift=-6em](polar) {Polar};
	\node [block, right of=centrallonepairs, yshift=-6em](polarbonds) {Polar bonds?};
	\node [block, right of=polarbonds, yshift=-6em](nonpolar) {Nonpolar};
