...
\begin{frame}{Chemical and Physical Change}
	...
	\end{itemize}
		\column{0.5\textwidth}
		\centering
		\only<1>{\includegraphics[scale=0.4]{07_04_Figure.jpg}}
		\only<2>{\includegraphics[scale=0.27]{07_05_Figure.jpg}}
	\end{columns}
\end{frame}
...
%	A substance
%	\begin{description}
%		\item[melts] when it changes from a solid to a liquid.
%		\item[freezes] when it changes from a liquid to a solid.
%		\item[sublimes] when it changes from a solid to a gas.
%		\item[deposits] when it changes from a gas to a solid.
%		\item[vaporizes] when it changes from a liquid to a gas.
%		\item[condenses] when it changes from a gas to a liquid.
%	\end{description}
\end{frame}
...
\begin{frame}[t]{Guide to Balancing a Chemical Reaction}
	...
	\begin{center}
		...
		\begin{tabular} {l c l}
			\ch{CH4\gas{}} \ch{+}
			\visible<3->{\alert{\ch{2}}}
			\ch{O2\gas{}} & \ch{->} &
			\ch{CO2\gas{}} \ch{+}
			\visible<3->{\alert{\ch{2}}} \ch{H2O\lqd{}} \\
		\end{tabular}
		}}

		\only<2->{\begin{tabular} {l c l}
			\ch{CH4\gas{}} \ch{+}
			\visible<3->{\alert{\ch{2}}}
			\ch{O2\gas{}} & \ch{->} &
			\ch{CO2\gas{}} \ch{+}
			...
			\\ \midrule
			1 C & \visible<4->{$=$} & 1 C \\
			4 H & \visible<4->{$=$} & \alt<4->{\alert{4}}{2}
			H \mode<presentation>{\visible<3>{$\times$ 2}} \\
			\alt<4->{\alert{4}}{2} O
			\mode<presentation>{\visible<3>{$\times$ 2}} & \visible<4->{$=$} &
			\alt<4->{\alert{4}}{3}\ O
			\mode<presentation>{\visible<3>{$\times$ 2}}
		\end{tabular}}
	\end{center}
\end{frame}

\begin{frame}[t]{Balance the following equation\ldots}
	\begin{reaction*}
...
\begin{frame}[t]{Limits in Chemical Reactions}
	...
	\only<4>{%
		\begin{center}
			\includegraphics[scale=0.4]{07_Pg300_UnFigure_1.jpg}
		\end{center}
	}
\end{frame}
