...
\begin{frame}{The Wave Nature of Light}{Diffraction}
	\centering
	\only<1>{\includegraphics[scale=0.38]{diffraction.jpeg}}

	\only<2>{\includegraphics[scale=0.38]{diffraction-interference.jpeg}}
\end{frame}
...
\begin{inclass}
	\parbox[t][14em]{\linewidth}{Calculate the energy of a photon with a
...
\begin{frame}{Emission Spectra}
	...
	\only<2>{\includegraphics<only@2>[scale=0.4]{emission-spectra2.jpg}}
\end{frame}
...
\begin{frame}{The Bohr Model and Emission Spectra}
	\centering

	\includegraphics[scale=0.45,trim={0 0 0 40pt},clip]{bohr-model.jpeg}
\end{frame}
...
\begin{frame}{Electron Transitions}
	\begin{columns}
		\column{0.5\linewidth}
		\begin{itemize}
			\item<1-> Electrons can exist \alert{only} in certain
				orbitals (not in between).
			\item<2-> Light \alert{emission} is a result of an
				\alert{electron transition} from a higher energy
				orbital to one of lower energy.
			\item<3-> Ligh \alert{absorption} is a result of an
	...
	\begin{columns}
		...
		\begin{center}
			...
			\begin{axis}[axis x line=middle,
				...
				xtick=\empty,
				domain=-0.1:8.5,]
				\addplot[color=primary,samples=200,ultra thick] {sin(x*90)};
				\only<2->{
		...
		\begin{center}
			...
			\begin{axis}[axis x line=middle,
				...
				xtick=\empty,
				domain=-0.1:8.5,]
				\addplot[color=primary,samples=200,ultra thick]
				{(sin(x*90))^2};
				\only<4->{
...
\begin{frame}{Translating Probability to 3D Orbitals}
	...
	\begin{columns}
		...
		\alt<2->{\includegraphics[scale=0.27]{1sradialdist.jpeg}}
